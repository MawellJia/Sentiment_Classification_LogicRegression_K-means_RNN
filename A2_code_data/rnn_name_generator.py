...
def seqs_to_ids(seqs, char_to_id, max_len=20):
    ...
    for name in seqs:
        ...
        for char in name:
            # No id should be longer than max_len
            if len(name_id) == max_len:
                break
            if char in char_to_id:
                name_id.append(char_to_id[char])
        # Add padding 0 if it's not longer than max_len
        if len(name_id) < max_len:
            for i in range(max_len - len(name_id)):
                name_id.append(0)

        # put every id for name in result list and reset name id
        all_seqs.append(name_id)
        name_id = []
...
class RNNLM(nn.Module):
    ...
    def forward(self, x, h_last=None):
        ...
        # Use it if there is a previous hidden state
        # Or we just use the GRU's default initial state
        if h_last is not None:
            out, h = self.gru(x, h_last)
        else:
            out, h = self.gru(x)

        # Convert GRU outputs to logits for each token in the vocabulary.
        out = self.linear(out)
...
def train_model(model, Xtrain, Ytrain, Xval, Yval, id_to_char, max_epoch):
    ...
    for e in range(max_epoch):
        ...
        for i in range(num_batches):
            ...
            # Forward pass, compute predicted logits from the model.
            logits, _ = model(x_batch)

            # Compute the cross-entropy loss.
            loss = lossfn(logits.view(-1, logits.size(2)), y_batch.view(-1))
            ...
            print(f"Epoch {e + 1}/{max_epoch}, Training loss: {total_loss / num_batches:.4f}")

        # Calculate and print the validation loss at the end of training.
    logits, _ = model(Xval)
    val_loss = lossfn(logits.view(-1, logits.size(2)), Yval.view(-1))
...
def gen_string(model, id_to_char, max_len=20, sample=True):
    ...
    for i in range(max_len):
        ...
        # Forward pass through the model to get logits and next hidden state.
        logits, h = model(x, h)

        # Apply softmax on logits to get the probability distribution.
        probs = torch.softmax(logits[:, -1, :], dim=-1)

        # Depending on the `sample` flag, either sample from the distribution or take the argmax.
        if sample:
            next_token = torch.multinomial(probs, num_samples=1)
        else:
            next_token = torch.argmax(probs, dim=-1, keepdim=True)

        # If the next token corresponds to the end-of-sequence token, break the loop.
        if next_token.item() == 2:  # <eos> token id is 2.
            break

        # Append the character corresponding to the token to the output string.
        out_str += id_to_char[next_token.item()]

        # Update x for the next iteration. Use view to ensure x remains 2-D.
